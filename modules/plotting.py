...
def plot_pca_scatter(
    ...
    if ranking_df is not None and "Sample" in df.columns:
        merge_cols = ["Sample"]
        if "Final_design_score" in ranking_df.columns:
            merge_cols.append("Final_design_score")
        if "Constraint_status" in ranking_df.columns:
            merge_cols.append("Constraint_status")
    ...
    color_col = "Final_design_score" if "Final_design_score" in df.columns else None
    ...
    fig.update_layout(
        xaxis_title=f"PC1 ({pc1:.2f}% variance)",
        yaxis_title=f"PC2 ({pc2:.2f}% variance)",
        height=650,
    )
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            range=[
                ...
                max(
                    105,
                    float(variance_df["Cumulative_percent"].max()) + 5
                )
            ]
        ),
    )
...
def plot_feature_contribution(contribution_df: pd.DataFrame):
    df = contribution_df.sort_values(
    ...
    fig = px.bar(
        ...
        y="Feature",
        orientation="h",
        text="Contribution_percent",
        title="Feature contribution to PC1/PC2",
    )
...
def plot_score_ranking(ranking_df: pd.DataFrame):
    ...
    fig = px.bar(
        top,
        x="Final_design_score",
        y="Sample",
        orientation="h",
        color="Constraint_status",
        title="Final design score ranking",
    )

    fig.update_layout(
        xaxis_title="Final design score",
        yaxis_title="Sample",
...
def plot_composition_fingerprint(
    df: pd.DataFrame,
    sample_name: str,
    ...
    plot_df = pd.DataFrame(
        {
            ...
            "Value": [row.get(c, 0) for c in feature_cols],
        }
    )
    ...
    fig = px.bar(
        ...
        x="Feature",
        y="Value",
        title=f"Composition fingerprint: {sample_name}",
    )
