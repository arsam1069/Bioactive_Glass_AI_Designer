...
def plot_explained_variance(variance_df: pd.DataFrame):
    fig = px.bar(variance_df, x="PC", y="Explained_variance_percent", text="Explained_variance_percent", title="Explained variance")
    fig.add_trace(go.Scatter(x=variance_df["PC"], y=variance_df["Cumulative_percent"], mode="lines+markers", name="Cumulative %"))
    fig.update_traces(texttemplate="%{text:.1f}%", textposition="outside")
    fig.update_layout(yaxis_title="Variance (%)", height=450)
    return fig
